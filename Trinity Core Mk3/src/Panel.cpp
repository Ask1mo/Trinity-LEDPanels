...
Panel::Panel(uint8_t panelNumber)
{
...
void Panel::Write()
{
  ...
  byte blueValue;
    ...
    {
      leds[((PanelNumber*LEDSPERLEAF)+i)] = CRGB(redValue, greenValue, blueValue);
    }
}

void Panel::Tick()
{
  if ((Offset) > timer)timer++;
  else
  {
    switch (Effect)
    {
  ...
  for(byte i = 0; i < Speed; i++)
  {
    ...
    switch (fxProgression)
    {
      ...
      case 3:
      {
        ...
        if(dummyValue == 40 && fxType == 3) fxProgression = fxProgression + 2;
  ...
  for(byte i = 0; i < Speed; i++)
  {
    ...
    switch (fxProgression)
    {
      ...
      case 2:
      {
        ...
        if(dummyValue == 0)
        {
          if(fxType == 5 || fxType == 11)fxProgression = fxProgression + 2;
          else fxProgression++;
        }
      }
      break;
  ...
  for(byte i = 0; i < Speed; i++)
  {
    ...
    switch (fxProgression)
    {
      ...
      case 1:
      {
        ...
        if(fxType == 7 && dummyValue == 100)fxProgression = fxProgression + 3;
...
void Panel::FX_sound()
{
  ...
  allowRed = getColourClearance(Effect, 0);
  allowGreen = getColourClearance(Effect, 1);
  allowBlue = getColourClearance(Effect, 2);
  ...
  if(globals->soundLevel > (Offset))
  {
    redValue = 255;
    if(globals->soundLevel<= globals->soundMaximum)
    {
      if(allowRed)redValue = 255-globals->soundLevel/globals->soundMaximum*255;
      if(allowGreen)greenValue = 255-globals->soundLevel/globals->soundMaximum*255;
      if(allowBlue)blueValue = 255-globals->soundLevel/globals->soundMaximum*255;
    }
    else
    {
      greenValue = 0;
  ...
  if(globals->soundPeak >= Offset && globals->soundPeak < Offset+Speed && Speed != 1)
  {
    redValue = 255;
    greenValue = 255;
    blueValue = 255;
